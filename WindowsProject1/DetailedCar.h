...
private:
    ...
    void drawLeftDoor() {
        const float hingeX = 1.0f;
        const float hingeY = 0.5f;
        const float hingeZ = -1.15f;
        const float doorLength = 1.8f;
        ...
        // Door handle
        glPushMatrix();
        glTranslatef(-1.3f, 0.0f, -doorThick * 2.0f);
        ...
        // Side mirror
        glPushMatrix();
        glTranslatef(doorLength/8.0f - 0.3f, 0.2f, -doorThick * 2.5f);
        glColor3f(0.1f, 0.1f, 0.1f);
